        ...
        <div className="relative w-full group">
            <div className="relative">
                <textarea
                    ...
                    className={` ${isError ? "border-[var(--form-error)]" : "border-[var(--main-color)] border-transparent "} resize-none z-20 relative w-full h-50 transition duration-300 ease-in-out border-2 border-solid focus:border-2 focus:bg-[var(--background)] focus:border-[var(--main-color)] pt-7 pl-4 pr-8 rounded-[.5rem] outline-none bg-[var(--form-bg)] text-[var(--foreground)]`}
                    name={name}
                    onFocus={handleFocus}
                ...
                />
                <label htmlFor={id}
                       className={`${hasValue ? "top-4 text-xs" : "top-7"} ${changeBg ? "bg-[var(--background)] pointer-events-none mx-4 w-10/11" : "bg-[var(--form-bg)] w-15/16 ml-4 "} z-20 h-7 flex items-center group-focus-within:text-xs group-focus-within:top-4 text-[var(--dark-gray)] absolute left-0 transform -translate-y-1/2 transition-all duration-300 ease-in-out`}>{label}</label>
